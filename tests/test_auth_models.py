...
from core.auth.models import (ApiKey, Base, Enterprise, License, Subscriber,
                              User)
...
def test_create_enterprise_subscriber(session):
    ...
    ent = Enterprise(name="Acme Corp", subscriber_id=sub.id)
    session.add(ent)
    session.commit()
...
def test_api_key_relationship(session):
    # Setup Hierarchy
    lic = License(
        key_hash="k", valid_from=datetime.utcnow(), valid_until=datetime.utcnow()
    )
    session.add(lic)
    session.commit()
    ...
    # Create API Key
    key = ApiKey(user_id=user.id, key_hash="hashed_key", prefix="sk_live_")
    session.add(key)
    session.commit()
