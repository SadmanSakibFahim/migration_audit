...
from core.db.drivers import (check_driver_installed, detect_db_type,
                             get_installation_command)
from core.db.exceptions import (DatabaseConnectionError, DatabaseDriverError,
                                DatabaseQueryError)
...
class TestDatabaseExceptions:
    ...
    def test_connection_error_network(self):
        ...
        error = DatabaseConnectionError(
            uri="postgresql://user:pass@localhost/db", original_error=original_error
        )
    ...
    def test_connection_error_table_not_found(self):
        ...
        error = DatabaseConnectionError(
            uri="sqlite:///test.db", original_error=original_error, table_name="users"
        )
...
class TestCustomQueries:
    ...
    def test_load_table_with_custom_query(
        self, mock_read_sql, mock_validate, mock_pool
    ):
        ...
        result = load_table(
            path="postgresql://user:pass@localhost/db/users", query=query
        )
    ...
    def test_load_table_without_custom_query(
        self, mock_read_sql, mock_validate, mock_pool
    ):
