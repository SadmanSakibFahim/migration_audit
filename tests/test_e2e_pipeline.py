...
class TestE2EHappyPath:
    ...
    def test_pipeline_produces_volume_check(self, sample_src_df, sample_tgt_df):
        ...
        volume_results = [
            r for r in results if "Volume" in r.name or "volume" in r.name.lower()
        ]
...
class TestE2ENoGo:
    ...
    def test_invalid_mapping_causes_no_go(self):
        ...
        meta = _meta(
            mappings=[
                MappingConfig(columns=["status"], allowed_values=["active", "inactive"])
            ],
        )
...
class TestE2EWarnings:
    ...
    def test_slight_volume_loss_warns(self):
        ...
        tgt_modified.loc[99, "id"] = (
            999  # 1 PK doesn't match source: 99% overlap → PASS
        )
...
class TestE2EFullDimension:
    def test_all_five_dimensions_run(self, sample_src_df, sample_tgt_df):
        ...
        assert "identity" in result_names or "Identity" in " ".join(
    ...
    def test_constraint_not_null_violation_detected(self):
        ...
        constraint_results = [
            r
            for r in results
            if "constraint" in r.name.lower() or "Constraint" in r.name
        ]
