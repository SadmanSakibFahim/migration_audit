...
def test_auth_user_flow(db_session):
    ...
    # Setup License & Sub
    lic = auth.create_license("hash_key", datetime.now(), datetime.now() + timedelta(days=365))
...
def test_license_expiry_check(db_session):
    ...
    # Valid License
    lic_valid = auth.create_license("valid", datetime.now() - timedelta(days=1), datetime.now() + timedelta(days=1))
    ...
    # Expired License
    lic_expired = auth.create_license("expired", datetime.now() - timedelta(days=10), datetime.now() - timedelta(days=1))
...
def test_rbac_permissions(db_session):
    ...
    lic = auth.create_license("rbac_key", datetime.now(), datetime.now() + timedelta(days=365))
    sub = auth.create_personal_subscriber(lic.id)
