...
from core.audit.config_models import TableConfig, AuditConfig
...
class ScenarioRunner:
    ...
    def run_scenario(self, scenario_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.generator.generate_data_for_config(audit_config_dict, self.temp_dir, row_count=scenario_config.get('row_count', 20))
    ...
    def _apply_mutation(self, mutation: Dict[str, Any]):
        ...
        if m_type == 'delete_row':
            ...
            if len(df) > 0:
                ...
                print(f"Applied mutation: Deleted {len(indices_to_drop)} rows from {table}")
            ...
            if len(df) > 0 and col in df.columns:
                ...
                df.at[idx, col] = val
                print(f"Applied mutation: Modified {table}.{col} from {old_val} to {val} at index {idx}")
    ...
    def _verify_results(self, results: List[Any], expected_failures: List[Dict[str, Any]]) -> bool:
        ...
        for exp in expected_failures:
            ...
            if len(found) >= exp_count:
                ...
                print(f"MISSED: Did not find expected failure matching '{pattern}' (Found {len(found)}, Expected {exp_count})")
                all_matched = False
