    ...
    for idx, row in df.iterrows():
        ...
        if hasattr(table_config, 'data_constraints') and table_config.data_constraints:
            for col, constraints in table_config.data_constraints.items():
                ...
                constraint_list = constraints if isinstance(constraints, list) else [constraints]
                ...
                if 'date' in constraint_list:
                    if not pd.isna(row[col]) and row[col] != '':
                        try:
                            ...
                            reasons.append(f"Column '{col}' has invalid date value: {row[col]}")
        ...
        if reasons:
            ...
            try:
                with open('.cursor\\debug.log', 'a', encoding='utf-8') as f:
                    f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"D","location":"row_validator.py:55","message":"Row validation failure","data":{"table_name":table_name,"row_index":int(idx),"reasons":reasons,"sample_values":{col:str(row[col])[:50] for col in df.columns[:5]}},"timestamp":int(__import__('time').time()*1000)}) + '\n')
            except: pass
        # #endregion
        
        # Check mapping constraints (only for target data)
        if not is_source and hasattr(table_config, 'mappings') and table_config.mappings:
            for mapping in table_config.mappings:
                for col in mapping.columns:
                    if col in df.columns:
                        if not pd.isna(row[col]) and row[col] not in mapping.allowed_values:
                            reasons.append(
            ...
            if pk_col in df.columns:
                ...
                if not pd.isna(row[pk_col]):
                    ...
                    if len(duplicates) > 1 and idx != duplicates.index[0]:
                        reasons.append(f"Duplicate primary key '{row[pk_col]}' in column '{pk_col}'")
