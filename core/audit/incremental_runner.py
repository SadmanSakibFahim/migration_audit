...
class IncrementalRunner:
        ...
        for chunk in chunks:
            ...
            for col in aggregates:
                if col in chunk.columns:
                    self.src_aggregates[col] += pd.to_numeric(
    ...
    def finalize(self) -> List[TestResult]:
        ...
        status = (
            CheckStatus.PASS if loss_pct <= self.volume_tolerance else CheckStatus.FAIL
        )
        self.results.append(
            TestResult(
                ...
                details={
                    "src_rows": self.src_row_count,
                    "tgt_rows": self.tgt_row_count,
                },
            )
        )
        ...
        for col in self.src_aggregates:
            ...
            status = (
                CheckStatus.PASS
                if diff_pct <= self.aggregate_tolerance
        ...
        if pk and self.src_pk_sample:
            ...
            overlap_pct = (
                (found_count / len(self.src_pk_sample) * 100)
                if self.src_pk_sample
                else 0
            )
            ...
            self.results.append(
                TestResult(
                    ...
                    message=message,
                    details={
                        "sample_size": len(self.src_pk_sample),
                        "found_in_target": found_count,
                    },
                )
            )
        ...
        if self.mapping_errors:
            self.results.append(
                TestResult(
                    ...
                    details={
                        "errors": list(set(self.mapping_errors))[:10]
                    },  # Unique samples
                )
            )
